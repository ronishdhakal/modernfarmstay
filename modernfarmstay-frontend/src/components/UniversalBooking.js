...
export default function UniversalBooking() {
  ...
  const calculateTotal = () => {
    ...
    return calculateNights() * parseFloat(selectedRoom.price);
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (nights < selectedRoom.min_stay_days) {
        throw new Error(`Minimum stay is ${selectedRoom.min_stay_days} night(s)`);
      }

      if (formData.num_guests > selectedRoom.max_guests) {
        throw new Error(`Maximum ${selectedRoom.max_guests} guests allowed`);
      }
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6 max-w-xl mx-auto">
      {/* <h3 className="text-2xl font-bold mb-6 text-gray-800">Book Your Stay</h3> */}
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <select
            ...
            required
            className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-[#54b435]"
          >
            <option value="">Choose a room</option>
            {rooms.map(room => (
              <option key={room.id} value={room.id}>
                {room.title} - NPR {room.price}/night
              </option>
            ))}
          </select>
        </div>
        ...
        {selectedRoom && formData.check_in && formData.check_out && (
          <div className="border-t pt-4 space-y-2 text-sm">
            ...
            <div className="flex justify-between font-semibold text-lg border-t pt-2">
              <span>Total:</span>
              <span className="text-[#54b435]">NPR {calculateTotal().toFixed(2)}</span>
            </div>
            <div className="text-center text-xs text-gray-500 pt-1">
              Check-in: {formatTime(selectedRoom.check_in_time)} | Check-out: {formatTime(selectedRoom.check_out_time)}<br />
              Minimum stay: {selectedRoom.min_stay_days} night{selectedRoom.min_stay_days > 1 ? "s" : ""}
            </div>
          </div>
        )}
